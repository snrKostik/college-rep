...
public:
	...
	inline int* initValues(int size, int* values) {
		int* arr = new int[size];
	...
	inline void outStr(string* arr, int size) {
		for (int i{ 0 }; i < size; i++)
			print << arr[i] << " ";
		print << e;
	}
	...
	inline string* insertElStr(string* arr, int size, int el, string insert) {
		for (int i{ 0 }; i < size; i++) {
			if (i == el)
				arr[i] = insert;
		}
	...
	inline void delZero(int* arr, int size){
		int* copyed = copy(arr, 10);
		
	}
	//inline int size(int* arr) {
	//	int size = 0;
	//	int* aaaa = arr;
	//	for (int count : aaaa) {
	//		count++;
...
struct DynamicArrayStr {
	string* data;
	int size;

	DynamicArrayStr(int s) : size(s) {
